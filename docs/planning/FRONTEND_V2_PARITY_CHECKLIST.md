...
- [X] session list が表示される
- [X] 新規 session 作成が動作する
- [X] session 切替で active session が更新される
- [ ] 履歴は全件常駐せず、直近 window のみ再取得される

### Input

- [X] mode 切替が `chat/search/agent` で動作する
...
- [X] `/api/config` の初期値が表示される
- [X] field 編集が local reducer に反映される
- [ ] save で partial patch が送信される
- [ ] language 変更が i18n に反映される

### Agent / Tool Confirmation

- [X] 右パネルに mode / connection / activity が反映される
- [X] tool confirmation request が表示される
- [X] `deny` / `once` / `always_until_expiry` が送信される
