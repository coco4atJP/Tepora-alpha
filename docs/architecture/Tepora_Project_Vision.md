...
*   **戦略2: エコシステムへの準拠 (MCP Native)**
    *   車輪の再発明を避け、AI界隈の標準的なプロトコル（MCP）やツールスキーマに準拠します。
    *   これにより、独自のハックに頼ることなく、AIモデルが持つ本来の能力（Instruction Followingなど）を自然な形で引き出します。
*   **戦略3: 制御フローの透明化 (Rust Orchestrator)**
    *   自律型AIの挙動をブラックボックス化せず、ユーザーが理解・介入できるようにします。
    *   Rust実装の実行フローとイベントログにより、「なぜその行動をとったのか」を追跡可能にし、信頼関係を技術的に担保します。
