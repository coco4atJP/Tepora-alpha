...
graph TD
    ...
    ROUTER -->|"search"| SEARCH[SearchNode - Quick Search]
    ROUTER -->|"search_agentic"| AGENTIC["AgenticSearchNode - Deep Search"]
...
Search モードは **Quick (`Search` モード) / Deep (`SearchAgentic` モード) を明示選択する設計**になっています。自動昇格ではなく、入力された `mode` (`Search` または `SearchAgentic`) によって、`RouterNode` が Quick (`SearchNode`) / Deep (`AgenticSearchNode`) を選びます。
...
| `GET` | `/api/workspace/tree` | 現在のプロジェクトのツリー取得 |
| `GET` | `/api/workspace/document/*path` | ドキュメント内容取得 |
| `PUT` | `/api/workspace/document/*path` | ドキュメント作成/更新 |
