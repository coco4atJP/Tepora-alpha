...
│   ├── workspace/              # プロジェクトおよびドキュメント管理APIモジュール
...
| `GET` | `/api/workspace/document/{path}` | ドキュメントの読み込み |
| `PUT` | `/api/workspace/document/{path}` | ドキュメントの書き込み |
| `POST` | `/api/workspace/directory/{path}` | ディレクトリの作成 |
| `POST` | `/api/workspace/rename/{path}` | パス（ファイル/ディレクトリ）の名称変更 |
