...
| `POST`   | `/api/workspace/projects/{project_id}/select` | 現在のプロジェクトを選択         |
| `GET`    | `/api/workspace/tree`                         | 現在のワークスペースツリーを取得 |
| `GET`    | `/api/workspace/document/*path`               | ドキュメント内容の読み込み       |
| `PUT`    | `/api/workspace/document/*path`               | ドキュメント内容の書き込み       |
| `POST`   | `/api/workspace/directory/*path`              | ディレクトリの作成               |
| `POST`   | `/api/workspace/rename/*path`                 | パスの名前変更                   |
