...
| **ベクトル演算**      | ndarray             | コサイン類似度計算 (reranker用) |
...
│   │   └── mod.rs              # モジュール公開
│   │
│   ├── models/                 # ModelManager facade + registry/discovery/download/metadata/selection/event/types
│   ├── workspace/              # ProjectHistoryStore / Workspace API
...
│   ├── a2a/                    # Agent-to-Agent 通信 (protocol.rs を含む)
│   ├── crdt/                   # PoCモジュール (テスト用)
...
│   │   └── setup/              # セットアップ画面
│   │
│   └── legacy/                 # archived V1 frontend (standalone pages / Zustand stores 等は維持)
│       ├── App.tsx             # レガシーアプリルート
│       ├── api/                # ルーターローダー等
...
| `ModelHub`      | モデル管理ハブ (`ModelCard` / `ModelDetailOverlay` を含む) |
