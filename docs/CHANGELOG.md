...
- Split backend setup, websocket, security-controls, config validation, context-controller, and search-agentic orchestration into smaller modules; removed `AppStateCompat` and implicit `Deref`; and split frontend settings/chat layout-state hooks into dedicated model, lifecycle, and registry modules.
