...
_MAX_MEMORY_JSON_BYTES = 4096

_EM_STATS_COMMAND_PREFIXES = ("/emstats", "/emstats_prof", "/emstats_char")
...
class AgentCore:
    ...
    def route_by_command(self, state: AgentState) -> Literal["agent_mode", "search", "direct_answer", "stats"]:
        ...
        if user_input.startswith('/agentmode'):
            ...
            print("Route: search")
            return "search"
        elif any(user_input.startswith(prefix) for prefix in _EM_STATS_COMMAND_PREFIXES):
            print("Route: stats (EM diagnostics)")
            return "stats"
        else:
    ...
    def _build_graph(self):
        ...
        workflow.add_conditional_edges(
            ...
            self.route_by_command,
            {
                ...
                "stats": _GraphNodes.DIRECT_ANSWER,
            }
        )
    ...
    def save_memory_node(self, state: AgentState) -> dict:
        ...
        try:
            ...
            if last_ai_message:
                ...
                compact_history = []
                for msg in chat_history[-_MAX_MEMORY_MESSAGES:]:
                    entry = {
                ...
                if len(history_json.encode("utf-8")) > _MAX_MEMORY_JSON_BYTES:
                    truncated = history_json.encode("utf-8")[:_MAX_MEMORY_JSON_BYTES]
                    history_json = truncated.decode("utf-8", errors="ignore")

                self.memory_system.save_episode(summary=last_ai_message.content, history_json=history_json)
        except Exception as e:
    ...
    def agent_reasoning_node(self, state: AgentState) -> dict:
        ...
        # --- 階層型プロンプト構築 (EM-LLM準拠) ---
        # 1. アテンションシンク
        attention_sink_prefix = "This is a professional agent session. The agent will now reason and act to complete the given order."
        
        # 2. システムプロンプト
        system_prompt_template = config.BASE_SYSTEM_PROMPTS["react_professional"]
        system_prompt = system_prompt_template.format(tools=config.format_tools_for_react_prompt(self.tool_manager.tools))
        
        # 3. オーダー
        order_plan_str = json.dumps(state.get("order", {}), indent=2, ensure_ascii=False)
        
        # 4. 長期記憶 (EM-LLMから検索された記憶)
        long_term_memory_str = state.get("synthesized_memory", "No relevant long-term memories found.")
        
        # 5. 短期記憶 (ReActループの作業履歴)
        short_term_memory_str = _format_scratchpad(state["agent_scratchpad"])
