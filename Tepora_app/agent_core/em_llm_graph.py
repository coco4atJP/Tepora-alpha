...
class EMEnabledAgentCore:
    ...
    def em_memory_retrieval_node(self, state) -> dict:
        ...
        try:
            # EM-LLMの2段階検索を実行
            recalled_events_dict = self.em_llm_integrator.retrieve_relevant_memories_for_query(state["input"])
    ...
    async def _form_memory_with_surprisal(self, logprobs: Dict[str, Any]) -> List[Any]:
        """
        驚き度（Surprisal）ベースで記憶を形成する。（論文の主要な方法）
        """
        print(f"  - Analyzing {len(logprobs['content'])} tokens using surprisal-based segmentation.")
        return await self.em_llm_integrator.process_logprobs_for_memory(
            logprobs['content']
        )
    ...
    async def _form_memory_with_semantic_change(self, state: Dict[str, Any], ai_response: str) -> List[Any]:
        ...
        return await self.em_llm_integrator.process_conversation_turn_for_memory(
    ...
    async def em_memory_formation_node(self, state) -> dict:
        ...
        try:
            ...
            if logprobs and logprobs.get("content"):
                formed_events = await self._form_memory_with_surprisal(logprobs)
            else:
