...
class EMLLMIntegrator:
    ...
    async def process_logprobs_for_memory(self, logprobs_content: List[Dict]) -> List[EpisodicEvent]:
        """
        ...
        try:
            # 1. logprobsから驚き度スコアを計算
            surprise_scores = self.segmenter.calculate_surprise_from_logprobs(logprobs_content)
