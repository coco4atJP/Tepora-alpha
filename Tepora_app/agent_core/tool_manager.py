...
class ToolManager:
    ...
    def _load_native_tools(self) -> List[BaseTool]:
        ...
        if not config.GOOGLE_SEARCH_ENABLED:
            logger.warning(
                "Google Custom Search API keys are not configured. Skipping native_google_search tool registration."
            )
            return []
        try:
            # 1. Google Custom Search APIツールの準備
    ...
    def cleanup(self):
        ...
        if self.mcp_client and hasattr(self.mcp_client, 'close_all_sessions'):
            logger.info("Closing all MCP client sessions...")
            try:
                self._run_coroutine(self.mcp_client.close_all_sessions())
            except Exception as e:
                logger.error("Error during MCP client cleanup: %s", e, exc_info=True)
