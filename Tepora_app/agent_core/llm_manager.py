            ...
            try:
                ...
                if self._active_process:
                    try:
                        ...
                        logger.warning("Server for '%s' did not terminate during cleanup. Forcing kill...", key)
                        try:
                            self._active_process.kill()
                            self._active_process.wait()
                        except Exception as kill_error:  # noqa: BLE001
                            logger.error("Failed to kill server for '%s' during cleanup: %s", key, kill_error, exc_info=True)
                    except Exception as terminate_error:  # noqa: BLE001
                        logger.error("Error while terminating server for '%s' during cleanup: %s", key, terminate_error, exc_info=True)
                        try:
                            self._active_process.kill()
                            self._active_process.wait()
                        except Exception as kill_error:  # noqa: BLE001
                            logger.error("Failed to kill server for '%s' after termination error: %s", key, kill_error, exc_info=True)
                    finally:
                        self._active_process = None
                raise
                    ...
                    try:
                        ...
                        if self._embedding_process:
                            try:
                                ...
                                logger.warning("Embedding server '%s' did not terminate during cleanup. Forcing kill...", key)
                                try:
                                    self._embedding_process.kill()
                                    self._embedding_process.wait()
                                except Exception as kill_error:  # noqa: BLE001
                                    logger.error("Failed to kill embedding server '%s' during cleanup: %s", key, kill_error, exc_info=True)
                            except Exception as terminate_error:  # noqa: BLE001
                                logger.error("Error while terminating embedding server '%s' during cleanup: %s", key, terminate_error, exc_info=True)
                                try:
                                    self._embedding_process.kill()
                                    self._embedding_process.wait()
                                except Exception as kill_error:  # noqa: BLE001
                                    logger.error("Failed to kill embedding server '%s' after termination error: %s", key, kill_error, exc_info=True)
                            finally:
                                self._embedding_process = None
                                self._embedding_llm = None
                                self._embedding_config = None
