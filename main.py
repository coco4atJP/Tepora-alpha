...
from agent_core.config import MCP_CONFIG_FILE
from agent_core.llm_loader import load_gemma_llm
...
def main():
    ...
    try:
        ...
        # 1a. LLMをロード: Gemma 3NモデルをGPUに展開
        llm = load_gemma_llm()
        
        # 1b. ツールマネージャーを初期化: MCPサーバー起動とツール発見
        tool_manager = ToolManager(config_file=MCP_CONFIG_FILE)
        tool_manager.initialize()
        
        # 1c. エージェントコアを構築し、実行可能なグラフ(app)を取得
        agent_core = AgentCore(llm=llm, tool_manager=tool_manager)
    ...
    try:
        while True:
            try:
                ...
                # 2g. ユーザーへの最終応答を表示
                # ReActループの結果がある場合はそれを表示
                if "agent_outcome" in final_state and final_state["agent_outcome"]:
                    print(f"\nAI: {final_state['agent_outcome']}")
                elif chat_history:
    ...
    # プログラム終了時にリソースを解放します
        ...
        if llm and hasattr(llm, 'unload_model'):
            llm.unload_model()
        ...
        if tool_manager and hasattr(tool_manager, 'cleanup'):
            try:
                ...
                print(f"Warning: Failed to cleanup tool manager: {e}")
