...
class AgentCore:
    ...
    def direct_answer_node(self, state: AgentState) -> dict:
        ...
        # Gemma-3Nをロード
        print("--- Node: Direct Answer (using Gemma 3N) ---")
        llm = self.llm_manager.get_gemma_3n()
        # ペルソナとシステムプロンプトを結合
        ...
        # グラフが履歴管理の責任を持つ
        # ユーザーの入力とAIの応答を履歴に追加して返す
        return {"chat_history": state["chat_history"] + [HumanMessage(content=state["input"]), response_message]}
    ...
    def summarize_search_result_node(self, state: AgentState) -> dict:
        ...
        # Gemma-3nをロード
        print("--- Node: Summarize Search Result (using Gemma 3N) ---")
        llm = self.llm_manager.get_gemma_3n()
    ...
    def synthesize_final_response_node(self, state: AgentState) -> dict:
        ...
        # Gemma-3nをロード
        print("--- Node: Synthesize Final Response (using Gemma 3N) ---")
        llm = self.llm_manager.get_gemma_3n()
        ...
        # ★★★ コード改善: 履歴更新ロジックをシンプルに ★★★
        # 現在のユーザー入力と、今生成したAIの応答を、これまでの履歴に追加して返す
        new_history = state["chat_history"] + [HumanMessage(content=state["input"]), response_message]
        print(f"Final chat history length: {len(new_history)}")
