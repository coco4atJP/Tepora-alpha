    ...
    try:
        ...
        for server_name, server_config in config_data.get('mcpServers', {}).items():
            ...
            if not all(k in server_config for k in ["command", "args"]):
                logger.warning(f"Skipping server '{server_name}': missing 'command' or 'args'.")
                continue

            # 4. StdioConnectionオブジェクトを構築
            # ★★★ 修正点: 'transport': 'stdio' を明示的に追加 ★★★
            connections[server_name] = StdioConnection(
                transport="stdio", # この行が重要です！
...
def format_tools_for_prompt_simple(tools: List[BaseTool]) -> str:
    """
    プロンプト用に、ツールのリストを「関数シグネチャ風」のテキストで整形する
    
    処理の流れ:
    1. 各ツールのargs_schemaから引数情報を抽出
    2. 引数名と型を「name: type」形式で結合
    3. ツール名、引数、説明を「- tool_name(args): description」形式で整形
    4. 全ツールを改行で結合して返却
    """
    tool_strings = []
    for tool in tools:
        # 1. Pydanticのargs_schemaから引数を抽出
