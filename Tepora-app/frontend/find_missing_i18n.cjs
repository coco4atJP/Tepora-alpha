...
walkDir('e:/Tepora_Project/Tepora-app/frontend/src', function (filePath) {
    ...
    lines.forEach((line, index) => {
        ...
        if (jpregex.test(line)) {
            ...
            let codePart = line;
            if (line.includes('//')) {
                codePart = line.substring(0, line.indexOf('//')).trim();
            }
            if (codePart && jpregex.test(codePart)) {
                results.push(`${filePath}:${index + 1}: ${codePart.trim()}`);
            }
        }
    });
});

fs.writeFileSync('missing_i18n.txt', results.join('\n'));
