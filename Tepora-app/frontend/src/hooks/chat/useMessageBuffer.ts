...
import { Message, ChatMode, WebSocketMessage } from '../../types';
...
export const useMessageBuffer = (
    ...
    const applyBufferedContent = useCallback((
        ...
        bufferedMeta: { mode?: ChatMode, agentName?: string, nodeId?: string } | null
    ): Message[] => {
    ...
    const handleChunk = useCallback((data: WebSocketMessage) => {
            ...
            if (chunkBufferRef.current) {
                ...
                setMessages(prev => {
                    ...
                    if (lastMessage?.role === 'assistant' && !lastMessage.isComplete &&
                        ...
                        return [
                            ...prev.slice(0, -1),
                            { ...lastMessage, content: lastMessage.content + oldContent, isComplete: true }
                        ];
                    }
                    return prev;
                });
