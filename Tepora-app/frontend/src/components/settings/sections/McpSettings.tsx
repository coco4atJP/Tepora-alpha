...
import { Database, Plus, Power, Trash2, RefreshCw, AlertCircle, CheckCircle, XCircle, Loader2, Search, ExternalLink } from 'lucide-react';
import { SettingsSection } from '../SettingsComponents';
import { useTranslation } from 'react-i18next';
import { useMcpServers, useMcpStore, McpServerStatus, McpStoreServer } from '../../../hooks/useMcp';
...
const McpSettings: React.FC = () => {
    ...
    const getStatusText = (serverStatus?: McpServerStatus) => {
        ...
        switch (serverStatus.status) {
            case 'connected':
                return t('settings.mcp.status.connected', { count: serverStatus.tools_count });
            case 'connecting':
    ...
    return (
        ...
        >
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-2 ml-4">
                                    ...
                                    <button
                                        ...
                                        aria-label={config.enabled ? t('settings.mcp.disable') : t('settings.mcp.enable')}
                                        title={config.enabled ? t('settings.mcp.disable') : t('settings.mcp.enable')}
                                    >
                                        <Power size={16} />
                                    </button>
            ...
            {/* Store Modal */}
            {showStore && (
                <McpStoreModal onClose={() => setShowStore(false)} onInstalled={refresh} />
            )}
        </SettingsSection>
    );
};
...
const McpStoreModal: React.FC<McpStoreModalProps> = ({ onClose, onInstalled }) => {
    const { t } = useTranslation();
    const { storeServers, loading, error, searchQuery, setSearchQuery, install } = useMcpStore();

    const [selectedServer, setSelectedServer] = useState<McpStoreServer | null>(null);
    const [step, setStep] = useState<'browse' | 'runtime' | 'config' | 'confirm'>('browse');
    ...
    return (
        <Modal
            ...
            onClose={onClose}
            title={
                step === 'browse' ? t('settings.mcp.store.title') :
                    step === 'runtime' ? t('settings.mcp.store.selectRuntime') :
                        step === 'config' ? t('settings.mcp.store.configure') :
                ...
                {step === 'runtime' && selectedServer && (
                    <div className="space-y-4">
                        ...
                        {selectedServer.packages.map((pkg, i) => (
                            ...
                            >
                                <div className="font-medium text-gray-100">{pkg.runtimeHint || 'npx'}</div>
                ...
                {step === 'confirm' && selectedServer && (
                    <div className="space-y-4">
                        <div className="bg-black/30 border border-white/10 rounded-xl p-4">
                            <h4 className="font-medium text-gray-100 mb-2">{selectedServer.name}</h4>
                            ...
                            <div className="text-sm">
                                ...
                                {Object.keys(envValues).length > 0 && (
                                    <div className="mt-2 text-gray-400">
                                        ...
                                        <span className="ml-2">{Object.keys(envValues).length} variable(s) configured</span>
                                    </div>
                                )}
                            </div>
                        </div>
