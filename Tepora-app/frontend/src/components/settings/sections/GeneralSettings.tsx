        ...
        >
            ...
            <div className="mt-8 pt-6 border-t border-white/5">
                <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="text-lg font-medium text-white">Inference Engine (llama.cpp)</h3>
                </div>
...
const InferenceEngineUpdate: React.FC = () => {
    ...
    const [updateInfo, setUpdateInfo] = React.useState<{ has_update: boolean; current_version: string; latest_version?: string } | null>(null);
    ...
    const checkUpdate = async () => {
        try {
            setChecking(true);
            const res = await fetch(`${getApiBase()}/api/setup/binary/update-info?t=${Date.now()}`, { headers: { ...getAuthHeaders() } });
        ...
        try {
            ...
            if (data.job_id) {
                ...
                const poll = setInterval(async () => {
                    const progRes = await fetch(`${getApiBase()}/api/setup/progress?job_id=${data.job_id}`, { headers: { ...getAuthHeaders() } });
        ...
        <div className="bg-black/20 rounded-lg p-4 border border-white/5">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <div className="font-mono text-green-400">{updateInfo?.current_version || 'Unknown'}</div>
                </div>

                <div className="flex gap-3 items-center">
                    {updateInfo?.has_update && (
                        <div className="text-right">
                            <div className="text-xs text-yellow-400">New Version Available</div>
