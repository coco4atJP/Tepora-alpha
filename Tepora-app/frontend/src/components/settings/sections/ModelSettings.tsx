...
import { Cpu, HardDrive, Settings2, List, Plus, MessageSquare, Wrench } from 'lucide-react';
...
const ModelSettings: React.FC<ModelSettingsProps> = ({ llmConfig, modelsConfig, onUpdateLlm, onUpdateModel }) => {
    ...
    const [modelRoles, setModelRoles] = useState<ModelRoles>({ character_model_id: null, executor_model_map: {} });
    ...
    const handleSelectCharacterModel = async (modelId: string) => {
        try {
            const res = await fetch(`${getApiBase()}/api/setup/model/roles/character`, {
                method: 'POST',
    ...
    const handleSelectExecutorModel = async (taskType: string, modelId: string) => {
        try {
            const res = await fetch(`${getApiBase()}/api/setup/model/roles/executor`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', ...getAuthHeaders() },
    ...
    const handleRemoveExecutorMapping = async (taskType: string) => {
        try {
            const res = await fetch(`${getApiBase()}/api/setup/model/roles/executor/${taskType}`, {
                method: 'DELETE',
                headers: getAuthHeaders()
            });
            if (res.ok) {
    ...
    const handleAddTaskType = async () => {
        ...
        setModelRoles(prev => ({
            ...prev,
            executor_model_map: { ...prev.executor_model_map, [newTaskType.trim()]: '' }
        }));
        setNewTaskType('');
    };
    ...
    const handleSelectEmbeddingModel = async (modelId: string) => {
        try {
            ...
            if (res.ok) {
                ...
                if (modelInList && modelInList.filename) {
    ...
    return (
        <div className="space-y-6">
            ...
            >
                <div className="space-y-4">
                    {Object.entries(modelRoles.executor_model_map).map(([taskType, modelId]) => (
                        <ModelSelectionRow
                            key={taskType}
                            label={`Executor: ${taskType}`}
                            description={taskType === 'default' ? "Default fallback executor model." : `Specialized model for ${taskType} tasks.`}
                            role="text"
                            ...
                            config={modelsConfig.text_model}
                            onUpdateConfig={(c) => onUpdateModel('text_model', c)}
                            onDelete={taskType !== 'default' ? () => handleRemoveExecutorMapping(taskType) : undefined}
                            icon={<Wrench size={20} />}
                        />
                    ))}
