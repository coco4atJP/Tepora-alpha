...
const SystemStatusPanel: React.FC<SystemStatusPanelProps> = ({ isConnected, memoryStats }) => {
    ...
    const totalMemoryEvents = (memoryStats?.char_memory?.total_events || 0) +
            ...
            <div className="space-y-3">
                ...
                <div className="flex items-center justify-between group hover:bg-white/5 rounded-lg p-2 -m-2 transition-all duration-200">
                    <div className="flex items-center gap-2.5">
                        <div className={`w-7 h-7 rounded-lg flex items-center justify-center ${isConnected
                    ...
                    <span className={`text-[11px] font-semibold px-2 py-0.5 rounded ${isConnected
                ...
                {systemStatus && (
                    <div className="flex items-center justify-between group hover:bg-white/5 rounded-lg p-2 -m-2 transition-all duration-200">
                        <div className="flex items-center gap-2.5">
                            <div className={`w-7 h-7 rounded-lg flex items-center justify-center ${systemStatus.initialized
                                ? 'bg-blue-500/10 text-blue-400'
                                : 'bg-gray-500/10 text-gray-400'
                                }`}>
                                <Activity className="w-3.5 h-3.5" />
                            </div>
                            <span className="text-[11px] text-gray-400 font-medium">{t('status.core_system')}</span>
                        </div>
                        <span className={`text-[11px] font-semibold px-2 py-0.5 rounded ${systemStatus.initialized
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[10px]">
                        {memoryStats.char_memory && (
                            <div className="text-center p-1.5 rounded bg-white/5">
                                <div className="text-gray-500 uppercase tracking-wider mb-0.5">{t('status.char')}</div>
                                <div className="text-blue-400 font-bold">{memoryStats.char_memory.total_events}</div>
                            </div>
                        )}
                        {memoryStats.prof_memory && (
                            <div className="text-center p-1.5 rounded bg-white/5">
                                <div className="text-gray-500 uppercase tracking-wider mb-0.5">{t('status.prof')}</div>
                                <div className="text-purple-400 font-bold">{memoryStats.prof_memory.total_events}</div>
                            </div>
                        )}
                    </div>
                </div>
