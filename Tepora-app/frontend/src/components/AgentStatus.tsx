            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar -mr-2 pr-2 relative space-y-4">
                {recentActivities.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full text-gray-600 space-y-2 opacity-50">
                        ...
                        <span className="text-xs font-mono uppercase tracking-widest">{t('agent.waiting')}</span>
                    </div>
                ) : (
                    recentActivities.map((step, index) => (
                        <div key={index} className="relative pl-6 group">
                            ...
                            {/* Node Indicator */}
                            <div className={`absolute left-0 top-1.5 w-[18px] h-[18px] rounded-full border flex items-center justify-center bg-black/50 z-10 transition-colors duration-300 ${
                                step.status === 'completed' ? 'border-green-500/50 text-green-500 shadow-[0_0_10px_rgba(34,197,94,0.2)]' :
                                step.status === 'processing' ? 'border-tea-500 text-tea-500 shadow-[0_0_10px_rgba(233,122,58,0.3)]' :
                            ...
                            <div className={`p-3 rounded-lg border backdrop-blur-sm transition-all duration-300 ${
                                ...
                                <div className="flex justify-between items-start gap-2 mb-1">
                                    <span className={`text-[10px] font-bold uppercase tracking-wider ${
                                        step.agent_name.includes('Planner') ? 'text-purple-400' :
                                        step.agent_name.includes('Search') ? 'text-cyan-400' :
                                        'text-tea-400'
                                    }`}>
                                        {step.agent_name}
                                    </span>
                                ...
                                <div className={`text-xs leading-relaxed ${
                                    ...
                                    <div className={`break-words ${step.status === 'processing' ? 'text-tea-100 font-semibold' : ''}`}>
                                        {step.details}
                                    </div>
                                </div>
