...
import { Settings, MessageSquare, Search, Bot } from 'lucide-react';
...
const DialControl: React.FC<DialControlProps> = ({ currentMode, onModeChange, onSettingsClick }) => {
    ...
    const [hoveredMode, setHoveredMode] = useState<{ mode: string, x: number, y: number } | null>(null);
    const { t } = useTranslation();

    const MODES: { mode: ChatMode; label: string; angle: number; icon: React.ElementType }[] = useMemo(() => [
        { mode: 'direct', label: t('dial.chat'), angle: -90, icon: MessageSquare },
    ...
    return (
        ...
        <div className="relative w-48 h-48 flex items-center justify-center select-none">
            ...
            {MODES.map((m) => {
                ...
                if (m.mode === 'search') positionStyle = { top: '15%', left: '50%', transform: 'translate(-50%, -50%)' };
                else if (m.mode === 'direct') positionStyle = { top: '50%', left: '15%', transform: 'translate(-50%, -50%)' };
                else if (m.mode === 'agent') positionStyle = { top: '50%', right: '15%', transform: 'translate(50%, -50%)' };
                ...
                return (
                    <div
                        ...
                        style={positionStyle}
                        className={`absolute cursor-pointer transition-all duration-300 z-20 group ${isSelected
                            ? 'text-gold-400 scale-110 drop-shadow-[0_0_8px_rgba(255,215,0,0.6)]'
                            : 'text-coffee-200/50 hover:text-gold-200 hover:scale-110'
                            }`}
                    >
                        <div className={`p-2 rounded-full transition-all duration-300 ${isSelected ? 'bg-gold-500/10' : 'group-hover:bg-white/5'}`}>
