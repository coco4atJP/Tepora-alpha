					...
					<div className="min-h-screen w-full flex items-center justify-center bg-[#050201] text-white p-6">
						<div className="w-full max-w-lg glass-tepora rounded-2xl border border-white/10 shadow-2xl p-6">
							<div className="flex items-center gap-3 mb-3">
								...
								<h1 className="text-lg font-semibold">{t("errorBoundary.title", "Something went wrong")}</h1>
							</div>

							<p className="text-sm text-gray-300">
								{t("errorBoundary.message", "The UI encountered an unexpected error. Reload the app to continue.")}
							</p>

							{this.state.error && (
								<details className="mt-4 text-xs text-gray-300 bg-black/30 rounded-xl border border-white/10 p-3">
									<summary className="cursor-pointer select-none text-gray-400">{t("errorBoundary.details", "Error details")}</summary>
