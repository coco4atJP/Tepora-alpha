    ...
    def save_memory_node(self, state: AgentState) -> dict:
        ...
        try:
            ...
            last_ai_message = next(
                (msg for msg in reversed(chat_history) if isinstance(msg, AIMessage)), None
            )
            ...
            if len(history_json.encode("utf-8")) > MemoryLimits.MAX_MEMORY_JSON_BYTES:
                history_json = truncate_json_bytes(history_json, MemoryLimits.MAX_MEMORY_JSON_BYTES)

            # Save to memory system
            self.memory_system.save_episode(
                summary=last_ai_message.content, history_json=history_json
            )
