...
class EMTwoStageRetrieval:
    ...
    def add_events(self, events: list[EpisodicEvent]):
        ...
        for event in events:
            ...
            if (
                event.representative_embeddings is not None
                and event.representative_embeddings.shape[0] > 0
            ):
            ...
            try:
                ...
                if avg_surprise is not None:
                    try:
                        ...
                        logger.debug(
                            "Skipping avg_surprise metadata due to conversion error: %s",
                            avg_surprise,
                        )
    ...
    def _similarity_based_retrieval(
        self, query_embedding: np.ndarray, ks: int
    ) -> list[EpisodicEvent]:
        """
    ...
    def _contiguity_based_retrieval(
        self, similarity_events: list[EpisodicEvent], kc: int
    ) -> list[EpisodicEvent]:
        """
