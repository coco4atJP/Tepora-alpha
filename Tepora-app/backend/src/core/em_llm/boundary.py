...
class EMBoundaryRefiner:
    ...
    def calculate_conductance(self, similarity_matrix: np.ndarray, boundaries: list[int]) -> float:
        ...
        try:
            ...
            for i in range(num_communities):
                ...
                external_weight = np.sum(similarity_matrix[start:end, :start]) + np.sum(
                    similarity_matrix[start:end, end:]
                )
    ...
    def refine_boundaries(
        ...
        attention_keys: np.ndarray | None = None,
    ) -> list[EpisodicEvent]:
        """
        ...
        # Fallback: refinement based on sentence embedding similarity
            ...
            logger.warning(
                "Neither attention keys nor context vectors available. Skipping boundary refinement."
            )
            return events
    ...
    def _evaluate_boundary_position(
        self, similarity_matrix: np.ndarray, boundaries: list[int]
    ) -> float:
        """
        ...
        if self.config.refinement_metric == "modularity":
            ...
            return -self.calculate_conductance(
                similarity_matrix, boundaries
    ...
    def _rebuild_events_from_boundaries(
        self, original_events: list[EpisodicEvent], boundaries: list[int]
    ) -> list[EpisodicEvent]:
        """
