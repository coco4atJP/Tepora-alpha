...
class EMEventSegmenter:
    ...
    def _get_sentence_tokenizer(self):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "Failed to download 'punkt' tokenizer data: %s", download_error, exc_info=True
                )
                return self._fallback_sentence_tokenizer()
        except Exception as load_error:
            logger.warning(
                "Unexpected error while loading NLTK 'punkt': %s", load_error, exc_info=True
            )
            return self._fallback_sentence_tokenizer()
    ...
    def segment_text_into_events(
        self, text: str, embedding_provider: EmbeddingProvider
    ) -> tuple[list[EpisodicEvent], np.ndarray | None]:
        """
    ...
    def _identify_event_boundaries(
        self, scores: list[float], sentences: list[str] | None = None
    ) -> list[int]:
        """
