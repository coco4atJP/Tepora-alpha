...
from src.core.em_llm import EMEventSegmenter, EMConfig, EMLLMIntegrator
from src.core.graph import AgentCore
from src.core import config as agent_config
from src.core.llm.model_registry import ModelRegistry
from src.core.llm.process_manager import ProcessManager
...
class TestEMEventSegmenter(unittest.TestCase):
    ...
    def test_fallback_tokenizer_used_when_punkt_unavailable(self):
        with patch('nltk.data.load', side_effect=LookupError('missing data')) as mock_load, \
...
class TestEMLLMIntegrator(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_process_logprobs_skips_invalid_entries(self):
        ...
        with patch('src.core.em_llm.segmenter.EMEventSegmenter._get_sentence_tokenizer', return_value=tokenizer_stub):
...
class TestAgentCoreRouting(unittest.TestCase):
    ...
    def test_route_stats_commands(self):
        ...
        try:
             result = self.core.route_by_command(state)
        except Exception:
             result = "error"
