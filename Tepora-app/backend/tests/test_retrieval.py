...
class TestEMTwoStageRetrieval(unittest.TestCase):
    ...
    def test_retrieve_relevant_events(self):
        ...
        sim_event = EpisodicEvent(tokens=["sim"], start_position=10, end_position=15, surprise_scores=[])
        
        # Mock contiguity retrieval results
        cont_event = EpisodicEvent(tokens=["cont"], start_position=15, end_position=20, surprise_scores=[])
        
        with patch.object(self.retrieval, '_similarity_based_retrieval', return_value=[sim_event]):
            with patch.object(self.retrieval, '_contiguity_based_retrieval', return_value=[cont_event]):
    ...
    def test_deduplicate_events(self):
        ...
        e2 = EpisodicEvent(tokens=["a"], start_position=0, end_position=1, surprise_scores=[]) # Duplicate
