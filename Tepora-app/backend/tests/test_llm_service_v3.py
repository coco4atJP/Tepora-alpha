...
from src.core.models.types import ModelInfo, ModelLoader, ModelModality, ModelConfig
...
class TestLLMServiceV3(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.llama_patcher = patch("src.core.llm.service.LlamaServerRunner", return_value=self.mock_llama_runner)
        self.ollama_patcher = patch("src.core.llm.service.OllamaRunner", return_value=self.mock_ollama_runner)
    ...
    async def test_get_client_by_id_llama(self):
        ...
        info = ModelInfo(
            id=model_id, name="Test Llama", loader=ModelLoader.LLAMA_CPP, 
            path="/tmp/model.gguf", modality=ModelModality.TEXT
        )
    ...
    async def test_get_client_by_id_ollama(self):
        ...
        info = ModelInfo(
            id=model_id, name="Test Ollama", loader=ModelLoader.OLLAMA, 
            path="llama3:latest", modality=ModelModality.TEXT
        )
        self.mock_manager.get_model.return_value = info
    ...
    async def test_get_client_by_role(self):
        ...
        info = ModelInfo(
            id=mock_id, name="Role Model", loader=ModelLoader.LLAMA_CPP, 
            path="/tmp/role.gguf", modality=ModelModality.TEXT
        )
        self.mock_manager.get_model.return_value = info
    ...
    async def test_eviction_mixed_loaders(self):
        ...
        # 1. Load Llama
        id1 = "llama-1"
        info1 = ModelInfo(id=id1, name="L1", loader=ModelLoader.LLAMA_CPP, path="p1", modality=ModelModality.TEXT)
        ...
        id2 = "ollama-2"
        info2 = ModelInfo(id=id2, name="O2", loader=ModelLoader.OLLAMA, path="p2", modality=ModelModality.TEXT)
        self.mock_manager.get_model.side_effect = lambda mid: info2 if mid == id2 else (info1 if mid == id1 else None)
