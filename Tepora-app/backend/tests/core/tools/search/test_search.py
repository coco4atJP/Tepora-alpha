...
class TestSearchTool:
    def test_run_success(self, mock_settings):
        ...
        mock_engine.search.return_value = [SearchResult(title="T1", url="U1", snippet="S1")]

        tool = SearchTool(engine=mock_engine, name="test_search", description="Test search tool")
    ...
    def test_privacy_disabled(self, mock_settings):
        ...
        tool = SearchTool(
            engine=MagicMock(spec=SearchEngine), name="test_search", description="Test search tool"
        )
