...
def test_validate_missing_models_section(mock_config, mock_root):
    mock_config.models_gguf = {}
    with pytest.raises(ValueError, match="models_gguf section is missing"):
        validate_startup_config(mock_config, mock_root)
...
def test_validate_model_file_not_found(mock_config, mock_root):
